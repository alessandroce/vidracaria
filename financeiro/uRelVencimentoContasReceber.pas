unit uRelVencimentoContasReceber;
...
type
  TFRelVencimentoContasReceber = class(TFRelatorioPadrao)
...
var
  FRelVencimentoContasReceber: TFRelVencimentoContasReceber;
...
procedure TFRelVencimentoContasReceber.bt_CancelarExecute(Sender: TObject);
begin
  inherited;
  Close;
end;

procedure TFRelVencimentoContasReceber.bt_LimparExecute(Sender: TObject);
begin
  inherited;
  IdCli := 0;
  Edit1.Clear;
...
procedure TFRelVencimentoContasReceber.bt_okExecute(Sender: TObject);
...
begin
  ...
  begin
    Filtro := Filtro + 'and ((par_datavencto < current_date) and (coalesce(par_pago,''N'') = ''N'') and (par_datavencto between '+sDataIni+' and '+sdataFim+'))'+#13;
    FiltroUsado := FiltroUsado + 'Vencidas: '+StringReplace(sDataIni,'''','',[rfReplaceAll])+' até '+StringReplace(sDataFim,'''','',[rfReplaceAll])+' | ';
  end;
  if RadioButton2.Checked then
  begin
    Filtro := Filtro + 'and ((par_datavencto >= current_date) and (coalesce(par_pago,''N'') = ''N'') and (par_datavencto between '+sDataIni+' and '+sdataFim+'))'+#13;
    FiltroUsado := FiltroUsado + 'A vencer: '+StringReplace(sDataIni,'''','',[rfReplaceAll])+' até '+StringReplace(sDataFim,'''','',[rfReplaceAll])+' | ';
  end;
  ...
  begin
    {pagar}sRelatorio := 'FIN008_VENCIMENTO_CONTAS_RECEBER';
...
procedure TFRelVencimentoContasReceber.bt_SelCliExecute(Sender: TObject);
begin
  inherited;
...
procedure TFRelVencimentoContasReceber.FormShow(Sender: TObject);
begin
  inherited;
