...
type
  TFVendaComissionada = class(TFPadrao)
...
var
  FVendaComissionada: TFVendaComissionada;
...
procedure TFVendaComissionada.barra_botao(pBotao: TAction);
...
procedure TFVendaComissionada.Act_Btn_NovoExecute(Sender: TObject);
begin
  inherited;
...
procedure TFVendaComissionada.Act_Btn_AlterarExecute(Sender: TObject);
begin
  inherited;
  ibCadastro.Edit;
...
procedure TFVendaComissionada.Act_Btn_ExcluirExecute(Sender: TObject);
begin
  inherited;
...
procedure TFVendaComissionada.Act_Btn_ImprimirExecute(Sender: TObject);
begin
  inherited;
  barra_botao(Act_Btn_Imprimir);
end;

procedure TFVendaComissionada.Act_Btn_CancelarExecute(Sender: TObject);
begin
  inherited;
  ibCadastro.Cancel;
...
procedure TFVendaComissionada.Act_Btn_GravarExecute(Sender: TObject);
begin
  inherited;
  ibCadastro.Post;
  DMConexao.IBTransacao.CommitRetaining;
...
procedure TFVendaComissionada.Act_Btn_SairExecute(Sender: TObject);
begin
  inherited;
  Close;
end;

procedure TFVendaComissionada.ComboBox1Change(Sender: TObject);
begin
  inherited;
  if DMConexao.IBTransacao.InTransaction then
    DMConexao.IBTransacao.CommitRetaining;
...
procedure TFVendaComissionada.FormShow(Sender: TObject);
begin
  inherited;
  FValorOriginal := 0;
  FValorQuitado := 0;
...
procedure TFVendaComissionada.btCAClienteClick(Sender: TObject);
begin
  inherited;
...
procedure TFVendaComissionada.btEXClienteClick(Sender: TObject);
begin
  inherited;
  if not(IbCadastroAtivo) then
    Exit;
...
function TFVendaComissionada.IbCadastroAtivo: Boolean;
begin
  Result := (ibCadastro.State in [dsEdit,dsInsert]);
end;

function TFVendaComissionada.getMesAnoMovimento(pMes, pAno: Integer): String;
...
procedure TFVendaComissionada.ibCadastroNewRecord(DataSet: TDataSet);
begin
  inherited;
